...
public class StreamExample {
	public static void main(String args[]) {
		...
		countries
			.stream()
			.filter(c -> c.getPopulation() > 10_000_000) // vraca stream drzava koje ispunjavaju kriterijum
			.forEach(c -> c.displayCountry());
		
		System.out.println("\nIspis naziva drzava koje imaju broj stanovnika veci od 10 miliona");
		countries
			.stream()
			.filter(c -> c.getPopulation() > 10000000) // Stream<Country>
			.map(c -> c.getName()) // vraca stream naziva drzava Stream<String>
